...
<html>
	<head>
		<title>index</title>
		<link rel="stylesheet" type="text/css" href="{{ URL::asset('assets/css/') }}/tambahan.css">
			...
			<h1 class="page-header">Lupa Password</h1>
                <div class="row">
                    <div class="col-md-offset-4 col-md-4">
                        <br/>
                        <form method="POST" action="{{ url('/lupa-ganti-proses') }}">
                            {!! csrf_field() !!}
                            <div class="row">
                                <div class="col-md-12">
	                                ...
	                                @if ($errors->has('passbaru'))
	                                    <span class="help-block alert-danger">
	                                        <strong>{{ $errors->first('passbaru') }}</strong>
	                                    </span>
	                                @endif
                                    <label>Password Baru Konfirmasi</label>
                                    <input type="password" class="form-control" name="passbaruConfirm">  
	                                @if ($errors->has('passbaruConfirm'))
	                                    <span class="help-block alert-danger">
	                                        <strong>{{ $errors->first('passbaruConfirm') }}</strong>
	                                    </span>
	                                @endif
                                    <br/>   
                                    <input type="hidden" class="form-control" value="{{$request['token_get']}}" name="token"> 
